# Wissen-Login - Task 
## (submitted on 10-July-2024)

A simple web app built with React JS to have an authentication system & hosting powered by Firebase, home screen

![image description](data:image/png;base64,)

#### Login Image:
![image description](https://github.com/user-attachments/assets/4a53eb42-03fd-46da-a3c1-d4dd1df4853d)
)

#### Home Page:
![image description](https://github.com/user-attachments/assets/78c812c5-bcda-45c4-8a70-24c2be2cfc9d)

## Tech Stack
| Library | Version  |
| :---:   | :---: |
| React | 18.2   |
| Sass | 1.72.x   |
| Firebase | 10.9.x   |






## Installation
Assuming you're all the pre requisites installed on your machine such as NodeJS, yarn or npm package manager.

#### Clone the repo

bash
  git clone https://github.com/sriesh-0204/Login.git
  cd Login
  
#### Live Link: 
    live link https://669e43b6308b9b283394f44f--sparkling-heliotrope-bdfa77.netlify.app/


#### Install dependenciesa

bash
  yarn (or) npm i
  
#### Start Server

npm run start
 


#### Start the server locally on your machine

bash
  yarn start


